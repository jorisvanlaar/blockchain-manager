...
class Blockchain:
    ...
    def save_data(self):
        ...
        try:
            with open('blockchain.txt', 'w') as file:
                ...
                file.write('\n')
                saveable_tx = [tx.__dict__ for tx in self.open_transactions] # Objecten kunnen niet als json worden opgeslagen, en aangezien de open_transactions een list aan Transaction objecten is, moet je die eerst converten naar een list aan bijv. dictionaries (want dat datatype is wel weg te schrijven naar JSON)
    ...
    def get_balance(self):
        ...
        tx_sender = [[tx.amount for tx in block.transactions if tx.sender == participant] for block in self.chain]    # nested list comprehension die de amount opvraagt van alle transactions van de ingegeven participant, en dit teruggeeft in een list-kopie
        
        # Fetch a list of all sent amounts for the given person (empty lists are returned if the person was NOT the sender)
        # This fetches sent amounts of open transactions (to avoid double spending)
        open_tx_sender = [tx.amount for tx in self.open_transactions if tx.sender == participant]  # verzamelt de amounts van een participant die in de open_transactions list staan, in een nieuwe list
    ...
    def mine_block(self): #  The node parameter stands for the computer that is mining the block
        ...
        copied_open_transactions = self.open_transactions[:]     # de open_transactons list kopieren dmv slicen, 
